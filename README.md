...
**Two package variants, built by `./package_extension.sh`**, because the stores disagree on `background.scripts`:
- `dist/fact-check-ml-extension-<version>.zip` -- for **Firefox/AMO**. Includes `background.scripts` alongside `background.service_worker`, since Firefox doesn't run MV3 background code as a service worker and needs `scripts` to run any background code at all (Chrome's `service_worker` key is just ignored there, with a harmless linter warning).
- `dist/fact-check-ml-extension-<version>-chromium.zip` -- for **Chrome Web Store and Edge Add-ons**. Same package minus `background.scripts`, which Chrome/Edge's Manifest V3 validator hard-rejects as invalid (not just a warning) when present alongside `service_worker`.
